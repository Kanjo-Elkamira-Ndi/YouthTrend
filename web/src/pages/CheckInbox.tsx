...
// The `mode` query param drives the copy: ?mode=verify | ?mode=reset (default)
...
const CheckInbox = () => {
  ...
  const heading    = isVerify ? "Verify your email"           : "Check your inbox";
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        ...
        >
          ...
          {/* Back link */}
          <Link
            to={isVerify ? "/signup" : "/forgot-password"}
            className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
          >
            <ArrowLeft className="h-4 w-4" />
            {isVerify ? "Back to sign up" : "Back to forgot password"}
          </Link>
        </motion.div>
      </main>
    </div>
